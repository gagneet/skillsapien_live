...
if($attached_file_cv!=''){
  $updatecc = "UPDATE `cvs` SET name='".$attached_file_cv."' WHERE user_id = '".$user_id."'";
}
...
echo '<script type="text/javascript" language="javascript">alert("SomeThing Wrong!");</script>';
	...
	$sqlc = "SELECT * FROM profiles WHERE user_id='".$user_id."' and `vertical_id`='".VERTICAL_ID."' ";
  ...
  <div class="canten">
    <div class="center_in">
     ...
     <div style="padding-left:20px;">
        ...
        <?php
			if(isset($_GET['update']) && $_GET['update'] == "sucess"){
				echo '<font style=" color:green;"> <center>Your  Linkedin Profile details updated sucessfully</center></font><br>';
			}
		...
		?>
		  ...
		  ?>  
            ...
            <form id="linkedin_connect_form" method="get" action="linkedin/linkedin_signup.php">
                    
                     <input type="hidden" name="<?php echo LINKEDIN::_GET_TYPE;?>" id="<?php echo LINKEDIN::_GET_TYPE;?>" value="initiate" />
                    <input type="submit"  value="" style="background-image:url(linkedin/linkedin-new.png);width:121px; height:32px;border:none;position:relative;cursor:pointer;">
                    </form>
      ...
      <div class="imageHolder" style="float:right; display:inline; box-shadow:0px 0px 5px rgba(0,0,0,.25); border:solid 2px #ffffff; margin-right:40px;">
	  <?php if($s->is_set('R_USER_ID')==true)
                  {?><img src="profile_img/<?php echo $data['photo']; ?>" alt="" width="150" />
                  <?php } elseif(isset($_SESSION['probid_linkedinid']) && $_SESSION['probid_linkedinid'] !=''){ ?>
                  <img src="<?php echo $datac['linkedin_photo']; ?>" alt="" width="150" /><?php } 
				  else{ ?>
                  <img src="profile_img/<?php echo $datac['photo']; ?>" alt="" width="150" />
				  <?php }?></div>
		 ...
		 if($s->is_set('USER_ID')==true)
          ...
          <?php }?>
           ...
           <label class="label_cl2"> Attach Profile Photo </label>
            ...
            <div style="margin-top:10px;"><img src="<?php echo $datac['linkedin_photo']; ?>" alt="" width="50" height="50"></div>
            <?php } else { ?>
