...
if($_SERVER['QUERY_STRING']=="edit_country" && $_REQUEST['data']=="data")
{
	...
	if($s->is_set('USER_ID')==true)	
	{
		 ...
		 $profile_sql='SELECT * FROM `profiles` WHERE `user_id`='.$s->value('USER_ID').' and vertical_id="'.VERTICAL_ID.'" and `status`   like  "'.ENABLE.'" ';
	  ...
	  $rates="select * from rates where user_id='".$s->value('USER_ID')."' and vertical_id='".VERTICAL_ID."'";
...
$C_data=mysql_fetch_array($C_run);
 ...
 $sql="select count(bidderid)  as total from bid where jobid='".$s->value('USER_ID')."' and vertical_id='".VERTICAL_ID."'";
	 $records=mysql_query($sql);
	   $rows=mysql_fetch_array($records);
                 
 $sql1="select count(bidderid)  as baccept from bid where jobid='".$s->value('USER_ID')."' and status='accept' and vertical_id='".VERTICAL_ID."'";
	 $records1=mysql_query($sql1);
	   $rows1=mysql_fetch_array($records1);
	   
	  $complete="select count(user_id)  as complete from interview_success_details where user_id='".$s->value('USER_ID')."' and interview_complete='1' and vertical_id='".VERTICAL_ID."'";
	 $complete1=mysql_query($complete);
	   $complete1ss=mysql_fetch_array($complete1);
	   
	    $jobwon="select count(user_id)  as job from interview_success_details where user_id='".$s->value('USER_ID')."' and status='1' and vertical_id='".VERTICAL_ID."'";
	 $jobwon1=mysql_query($jobwon);
	   ...
	   $ddate = date('Y-m-d');
	   $view="select * from view_profile_total where user_id='".$s->value('USER_ID')."' AND date BETWEEN '". $views ."' AND '". $ddate."'";
...
$member="select * from user_login_details where id='".$s->value('USER_ID')."' and vertical_id='".VERTICAL_ID."'";
      ...
      <div class="sign_cl space_mn">
        ...
        <div class="form_area2">
          <div class="row2">
            ...
            <?php if(isset($_SESSION['probid_linkedinid']) && $_SESSION['probid_linkedinid'] !=''){?>
            <tr style="height:70px;text-align:center;color:#03F">
            	<td colspan="3">If you want to login using skillsapien username password, Please update your account with proper Email and password.</td>
            </tr>
            <?php } ?>
            <tr>
