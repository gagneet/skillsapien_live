    ...
    <div class="center_in">
       ...
       <tr>
                <td colspan="2" align="left" valign="top" class="font-13"><br/><br/><table width="100%" border="0" cellspacing="0" cellpadding="0">
                 <?php 
			 $sql="select count(bidderid)  as total from bid where bidderid='".$user_id."' and `vertical_id`='".VERTICAL_ID."'";
			     $records=mysql_query($sql);
				   $rows=mysql_fetch_array($records);
				 
	               ?>
                 <?php 
			 $sql1="select count(bidderid)  as baccept from bid where bidderid='".$user_id."' and status='accept' and `vertical_id`='".VERTICAL_ID."'";
			     $records1=mysql_query($sql1);
				   $rows1=mysql_fetch_array($records1);
	               ?>
      <?php
	       $sql12="select count(bidderid)  as breject from bid where bidderid='".$user_id."' and status='reject' and `vertical_id`='".VERTICAL_ID."'";
			     $records12=mysql_query($sql12);
				   $rows12=mysql_fetch_array($records12);
      ?>
       <?php
	       $sql123="select count(bidderid)  as counter from bid where bidderid='".$user_id."' and status='counter' and `vertical_id`='".VERTICAL_ID."'";
			     $records123=mysql_query($sql123);
				   $rows123=mysql_fetch_array($records123);
				   
				   	   
				$conduct="select count(recruiter_id)  as conduct from interview_success_details where recruiter_id='".$user_id."' and interview_complete='1' and `vertical_id`='".VERTICAL_ID."'";
			     $conduct_result=mysql_query($conduct);
				   $conduct_row=mysql_fetch_array($conduct_result);
				   
	$made="select count(recruiter_id)  as made from interview_success_details where recruiter_id='".$user_id."' and interview_complete='1' and status='1' and `vertical_id`='".VERTICAL_ID."'";
			     $made_result=mysql_query($made);
				   $made_row=mysql_fetch_array($made_result);
				   
				  $active="select count(recruiter_id)  as active from interview_success_details where recruiter_id='".$user_id."' and status='0' and `vertical_id`='".VERTICAL_ID."'";
		 ...
		 if($uuid>0)
			  {
				  ...
				  $history_split = new splitPageResults("SELECT * FROM `bid` WHERE  `vertical_id`='".VERTICAL_ID."' and `jobid` =".$s->value('USER_ID') ,MAX_DISPLAY_SEARCH_RESULTS, MAX_DISPLAY_ORDER_HISTORY);//Paging
			  }
			  if($s->value('R_USER_ID')>0)
			  {
			 	$user_id=$s->value('R_USER_ID');
				  $history_split = new splitPageResults("SELECT * FROM `bid` WHERE  `vertical_id`='".VERTICAL_ID."' and `bidderid` =".$s->value('R_USER_ID') ,MAX_DISPLAY_SEARCH_RESULTS, MAX_DISPLAY_ORDER_HISTORY);//Paging
			  }
...
<?php 
$bid_query="select * from bid where bidderid='".$user_id."' and `vertical_id`='".VERTICAL_ID."'";
...
$sql="select * from rates where user_id='".$jobid."' and `vertical_id`='".VERTICAL_ID."'";
...
$query="select * from skills_profiles where user_id='".$jobid."' and `vertical_id`='".VERTICAL_ID."'";
 $result=$db->query_silent($query);
 $profile=$db->fetch_array($result);
  ?>
  <tr <?php if($j%2=="0"){ echo "style='background-color:#E6F2F7;'"; }else{ echo ""; }?>>
  ...
  <?php }?>        
     ...
     <?php $rates="select * from rates where user_id='".$user_id."' and `vertical_id`='".VERTICAL_ID."'";
...
$C_data=mysql_fetch_array($C_run);
 ...
 $sql="select count(bidderid)  as total from bid where jobid='".$user_id."' and `vertical_id`='".VERTICAL_ID."'";
	 $records=mysql_query($sql);
	   $rows=mysql_fetch_array($records);
                 
 $sql1="select count(bidderid)  as baccept from bid where jobid='".$user_id."' and status='accept' and `vertical_id`='".VERTICAL_ID."'";
	 $records1=mysql_query($sql1);
	   $rows1=mysql_fetch_array($records1);
	   
	     $complete="select count(user_id)  as complete from interview_success_details where user_id='".$user_id."' and interview_complete='1' and `vertical_id`='".VERTICAL_ID."'";
	 $complete1=mysql_query($complete);
	   $complete1ss=mysql_fetch_array($complete1);
	   
	    $jobwon="select count(user_id)  as job from interview_success_details where user_id='".$user_id."' and status='1' and `vertical_id`='".VERTICAL_ID."'";
	 $jobwon1=mysql_query($jobwon);
	   $jobwon12=mysql_fetch_array($jobwon1);
	   
	   
	   
				 $fetch="select * from interview_success_details where status='1' and user_id='".$user_id."' and `vertical_id`='".VERTICAL_ID."'";
    ...
    <td colspan="2" align="left" valign="top" class="font-13"><br/><br/><table width="100%" border="0" cellspacing="0" cellpadding="0">
       ...
       <th class="nosort" width="5%"><h4>From</h4></th>
          ...
          <?php 
$bid_query="select * from bid where jobid='".$user_id."' and `vertical_id`='".VERTICAL_ID."'";
$bid_result=mysql_query($bid_query);
$i=0;
$j="0";
while($bid=mysql_fetch_array($bid_result))
...
$sqls="select * from post_job where user_id='".$bidderid."' and `vertical_id`='".VERTICAL_ID."'";
...
$profile_query="select * from profiles where user_id='".$bidderid."' and `vertical_id`='".VERTICAL_ID."'";
