...
class db_main
{
	...
	function init()
	{
		...
		$this->con=$this->connect(host, username, password);
	...
	function select_db($database)
	{
		...
		return $result = @mysql_select_db($database);
			exit;
		if (!$result)
		{
	...
	function query ($query, $debug_output = false, $die = true,$redirect='')
	{
		$this->init();
		if ($debug_output)
		{
		...
		if (!$result)
		{
			...
			//header_redirect('error_page.php?err=sql_err');
		}
		else 
		{
			return $result;
...
class database extends db_main
{
	...
	function get_sql_row ($query, $debug = false, $die = false, $redirect='')
	{
		(array) $row_result = NULL;

		 $query_result = $this->query($query, $debug, $die, $redirect);
