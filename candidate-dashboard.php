...
if($_SERVER['QUERY_STRING']=="edit_country" && $_REQUEST['data']=="data")
{
	...
	if($s->is_set('USER_ID')==true)	
	{
	  ...
	  $rates="select * from rates where user_id='".$s->value('USER_ID')."'";
...
$C_data=mysql_fetch_array($C_run);
 ...
 $sql="select count(bidderid)  as total from bid where jobid='".$s->value('USER_ID')."'";
	 $records=mysql_query($sql);
	   $rows=mysql_fetch_array($records);
                 
 $sql1="select count(bidderid)  as baccept from bid where jobid='".$s->value('USER_ID')."' and status='accept'";
	 $records1=mysql_query($sql1);
	   $rows1=mysql_fetch_array($records1);
	   
	  $complete="select count(user_id)  as complete from interview_success_details where user_id='".$s->value('USER_ID')."' and interview_complete='1'";
	 $complete1=mysql_query($complete);
	   $complete1ss=mysql_fetch_array($complete1);
	   
	    $jobwon="select count(user_id)  as job from interview_success_details where user_id='".$s->value('USER_ID')."' and status='1'";
...
$member="select * from user_login_details where id='".$s->value('USER_ID')."'";
		...
		echo	$C_sql='SELECT `countries_id`, `countries_name` FROM `countries` WHERE 1 ORDER BY `countries`.`countries_name` ASC';
            ...
            <div class="clear"></div>
              ...
              <tr>
                ...
                <?php 
				 $fetch="select * from interview_success_details where status='1' and user_id='".$profile_info->get_id()."'";
